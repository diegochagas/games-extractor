...
for (const [g, list] of siteGroups) {
  ...
  for (const ss of list) {
    ...
    const aliasName = SITE_ALIAS[norm(ss.character || '').replace(/[^a-z0-9 ]+/g, ' ').replace(/\s+/g, ' ').trim()] || ss.character;
    const rs = rendersFor(ss.character || '', CHAR_ZH[aliasName] || [], 6); rs.forEach((it, i) => { for (const v of (i === 0 ? ['front', 'side', 'back'] : ['front'])) if (it.files[v]) files.push({ file: it.files[v], label: it.pt + (v === 'front' ? '' : ' (' + VIEW_PT[v] + ')') }); });
